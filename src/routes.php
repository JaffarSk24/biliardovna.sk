...
$router->get('/cafe', function () use ($language) {
    ...
    $target = ($language === 'sk') ? '/' : "/{$language}";
    header("Location: " . $target);
    exit; /*(new PageController($language))->cafe();*/
});
...
$router->get('/admin/promo/new', function () use ($language) {
    (new AdminPromoController($language))->create();
});
$router->get('/admin/promo/edit', function () use ($language) {
    (new AdminPromoController($language))->edit();
});
$router->post('/admin/promo/save', function () use ($language) {
