...
function getPromoForEdit(PDO $pdo, ?int $id): array {
    if (!$id) {
        return [
            'item' => [
                'id' => null,
...
function savePromo(PDO $pdo, array $item, array $promo): int {
    $pdo->beginTransaction();
    try {
        $id = $item['id'] ? (int)$item['id'] : null;

        if (slugExists($pdo, $item['slug'], $id)) {
...
function getActivePromoForService(?int $serviceId, string $language = 'sk'): ?array {
